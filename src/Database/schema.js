...
function initializeSchema(db) {
  ...
  db.run(`CREATE INDEX IF NOT EXISTS idx_adjustments_product ON inventory_adjustments(product_id);`);
  ...
  const adminCheck = db.exec("SELECT COUNT(*) as cnt FROM users WHERE username = 'admin'");
  const adminExists = adminCheck.length > 0 && adminCheck[0].values[0][0] > 0;
  ...
  const catCount = catCheck.length > 0 ? catCheck[0].values[0][0] : 0;
...
function generateReceiptNumber(db) {
  ...
  const result = db.exec("SELECT value FROM settings WHERE key = 'receipt_counter'");
  let counter = 0;
  if (result.length > 0) {
    counter = parseInt(result[0].values[0][0]) || 0;
  }
  counter++;

  db.run("UPDATE settings SET value = ? WHERE key = 'receipt_counter'", [counter.toString()]);
